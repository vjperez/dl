jQuery(document).ready(function(){
	...
	.done(function(logueadoRespuesta, estatusForDONE, xhrObjetoForDONE){
		...
		switch(look){
			case'busca':
				if(!logueado)     { jQuery('#navLogin').css('visibility','visible').show();     jQuery('#navSignup').css('visibility','visible').show(); }
				else              { jQuery('#navLogout').css('visibility','visible').show();    jQuery('#navHome').css('visibility','visible').show()  ; }
