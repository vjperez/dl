...
if( isset($_SESSION['dueno_id']) && isset($_SESSION['own_nepes_with_ids']) ){
	...
	if($recurso){
		...
		if($recurso){
			if( pg_fetch_row($recurso) ){//updated nepe and insert video url
				...
				if($recurso){
					...
					pg_close($cnx);
					throw new Exception('Mal query.  Sin RECURSO, para preparadoQueryUp-sertVideo en: '  . __FILE__ );
				}				
			}else{						//updated nepe and update video
