...
if(isset($_SESSION['dueno_id'])){
	...
	require_once '../read/getSocialContactosQuery.php';
	$recurso = pg_execute($cnx, "preparadoQueryGetSocialContactos", array($dueno_to_query));
	if($recurso){
		$social_array = array();
		...
		pg_close($cnx);
		throw new Exception('Mal query.  Sin RECURSO para preparadoQueryGetSocialContactos en: ' . __FILE__  );
	}
}else{
